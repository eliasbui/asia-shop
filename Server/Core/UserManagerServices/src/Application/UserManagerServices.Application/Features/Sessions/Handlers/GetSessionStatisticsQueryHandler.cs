...
/// </summary>
public class GetSessionStatisticsQueryHandler : IRequestHandler<GetSessionStatisticsQuery, BaseResponse<SessionStatistics>>
{
    ...
    public async Task<BaseResponse<SessionStatistics>> Handle(GetSessionStatisticsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Getting session statistics for user {UserId}", request.UserId);

            var statistics = await _sessionManagementService.GetSessionStatisticsAsync(request.UserId, cancellationToken);

            _logger.LogInformation("Successfully retrieved session statistics for user {UserId}. Total: {Total}, Active: {Active}", 
        ...
        catch (Exception ex)
        {
            ...
            return BaseResponse<SessionStatistics>.Failure("An error occurred while retrieving session statistics. Please try again.",
