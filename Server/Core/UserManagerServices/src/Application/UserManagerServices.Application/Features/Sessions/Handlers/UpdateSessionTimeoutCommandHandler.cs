...
public class UpdateSessionTimeoutCommandHandler : IRequestHandler<UpdateSessionTimeoutCommand, BaseResponse<bool>>
{
    ...
    public async Task<BaseResponse<bool>> Handle(UpdateSessionTimeoutCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var securitySettings = await _unitOfWork.UserSecuritySettings.GetUserSecuritySettingsAsync(request.UserId, cancellationToken);
            ...
            await _unitOfWork.UserSecuritySettings.CreateOrUpdateUserSettingsAsync(request.UserId, securitySettings, cancellationToken);

            _logger.LogInformation("Successfully updated session timeout settings for user {UserId}. Timeout: {Timeout}min, Max Sessions: {MaxSessions}", 
